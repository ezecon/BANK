...
export default function LoanRequest() {
    ...
    const fetchLoanHistory = async () => {
      try {
        ...
        if (response.status === 200 && response.data.valid) {
          ...
          const loanHistoryResponse = await axios.get(`http://localhost:8000/api/loan/all`);
  ...
  const handleApprove = async (loanID, Status) => {
    try {
      const response = await axios.put(`http://localhost:8000/api/loan/approve/${loanID}`, { status: Status });
      ...
      alert("Failed to approve the loan request. Please try again.");
    }
  };
